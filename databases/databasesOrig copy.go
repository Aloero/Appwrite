...
func (srv *Databases) ListDocuments(DatabaseId string, CollectionId string, offsetQueries []string) (*models.DocumentList, error) {
	const maxLimit = 4000
	var offset = 0

	// optionalSetters := []ListDocumentsOption{srv.WithListDocumentsQueries(offsetQueries)}

	path := fmt.Sprintf("/databases/%s/collections/%s/documents", DatabaseId, CollectionId)
	// fmt.Printf("path: %s\n", path)

	var resultDocumentList models.DocumentList

	for {
		offsetQueriesNew := append(offsetQueries, query.Offset(offset), query.Limit(maxLimit))
		optionalSetters := []ListDocumentsOption{srv.WithListDocumentsQueries(offsetQueriesNew)}
		...
		if !strings.HasPrefix(resp.Type, "application/json") {
			return nil, fmt.Errorf("не json ответ")
		}
		bytes := []byte(resp.Result.(string))
